...
#import "JKUUID.h"
#import "JKCKeyChainStore.h"

@implementation JKUUID

NSString *const FCUUIDsOfUserDeviceDidChangeNotification = @"FCUUIDsOfUserDeviceDidChangeNotification";

NSString *const _uuidForInstallation = @"fc_uuidForInstallation";
NSString *const _uuidForDeviceKey_jk = @"fc_uuidForDevice";
NSString *const _uuidsOfUserDevices = @"fc_uuidsOfUserDevices";
NSString *const _uuidsOfUserDevicesToggle = @"fc_uuidsOfUserDevicesToggle";


+(JKUUID *)sharedInstance
{
    static JKUUID *instance = nil;
...
-(NSString *)_getValueForKey:(NSString *)key userDefaults:(BOOL)userDefaults keychain:(BOOL)keychain service:(NSString *)service accessGroup:(NSString *)accessGroup
{
    ...
    if(!value && keychain ){
        value = [JKCKeyChainStore stringForKey:key service:service accessGroup:accessGroup];
    }
...
-(void)_setValue:(NSString *)value forKey:(NSString *)key userDefaults:(BOOL)userDefaults keychain:(BOOL)keychain service:(NSString *)service accessGroup:(NSString *)accessGroup synchronizable:(BOOL)synchronizable
{
    ...
    if( value && keychain ){
        JKCKeyChainStore *keychain = [JKCKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
-(NSString *)uuidForInstallation
{
    if( _uuidForInstallation == nil ){
        _uuidForInstallation = [self _getOrCreateValueForKey:_uuidForInstallation defaultValue:nil userDefaults:YES keychain:NO service:nil accessGroup:nil synchronizable:NO];
    }
...
-(void)uuidForDevice_updateWithValue:(NSString *)value
{
    ...
    [self _setValue:_uuidForDevice forKey:_uuidForDevice userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:NO];
}
...
-(NSString *)uuidForDevice
{
    ...
    if( _uuidForDevice == nil ){
        _uuidForDevice = [self _getOrCreateValueForKey:_uuidForDeviceKey_jk defaultValue:nil userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:NO];
    }
...
{
    _uuidsOfUserDevices_CloudAvailable = NO;
    ...
    if(NSClassFromString(@"NSUbiquitousKeyValueStore"))
    {
        ...
        if(iCloud)
        {
            _uuidsOfUserDevices_CloudAvailable = YES;
...
{
    if( _uuidsOfUserDevices_CloudAvailable )
    {
        NSUbiquitousKeyValueStore *iCloud = [NSUbiquitousKeyValueStore defaultStore];
        ...
        {
            NSString *uuidOfUserDeviceAsKey = [NSString stringWithFormat:@"%@_%@", _uuidForDevice, uuidOfUserDevice];
        ...
        [iCloud setBool:![iCloud boolForKey:_uuidsOfUserDevicesToggle] forKey:_uuidsOfUserDevicesToggle];
        [iCloud synchronize];
    }
}
...
-(void)uuidsOfUserDevices_iCloudChange:(NSNotification *)notification
{
    if( _uuidsOfUserDevices_CloudAvailable )
    {
        ...
        [iCloudDict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            ...
            if([uuidKey rangeOfString:_uuidForDevice].location == 0)
            {
        ...
        if([uuidsSet count] > uuidsCount)
        {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:FCUUIDsOfUserDeviceDidChangeNotification object:self userInfo:userInfo];
        }
    }
}
...
{
    _uuidsOfUserDevice = [value componentsJoinedByString:@"|"];
    [self _setValue:_uuidsOfUserDevices forKey:_uuidsOfUserDevices userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:YES];
}
...
-(NSArray *)uuidsOfUserDevices
{
    if( _uuidsOfUserDevices == nil ){
        _uuidsOfUserDevice = [self _getOrCreateValueForKey:_uuidsOfUserDevices defaultValue:[self uuidForDevice] userDefaults:YES keychain:YES service:nil accessGroup:nil synchronizable:YES];
    }
