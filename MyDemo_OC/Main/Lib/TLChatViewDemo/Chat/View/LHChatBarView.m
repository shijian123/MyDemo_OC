...
- (void)layoutSubviews {
    ...
    self.textView.frame = CGRectMake(textViewX, 7.5, MainScreenWidth - textViewX - 10, self.height - 15);
}
...
- (void)keyboardWillShow:(NSNotification *)notice {
    ...
    _keyboardHeight = (endFrame.origin.y != MainScreenHeight) ? endFrame.size.height:0;
    ...
    [UIView animateWithDuration:_animationDuration delay:0 options:(_animationCurve << 16 | UIViewAnimationOptionBeginFromCurrentState) animations:^{
        // 修改frame
        self.y = MainScreenHeight - self.height - _keyboardHeight;
...
//        [_conversationChatVC scrollToBottomAnimated:NO refresh:NO];
    ...
    if (self.emojiKeyboard) { // 当前展示的是表情键盘
        [UIView animateWithDuration:_animationDuration delay:0 options:(_animationCurve << 16 | UIViewAnimationOptionBeginFromCurrentState) animations:^{
            self.emojiView.y = MainScreenHeight - kChatEmojiHeight;
            [self.superview layoutIfNeeded];
        } completion:^(BOOL finished) {
            self.emojiView.y = MainScreenHeight;
            self.moreView.y = MainScreenHeight - kChatMoreHeight;
...
- (void)keyboardWillHide:(NSNotification *)noti {
    ...
    [UIView animateWithDuration:duration delay:0 options:(curve << 16 | UIViewAnimationOptionBeginFromCurrentState) animations:^{
        self.y = MainScreenHeight - self.height;
        _tableView.height = self.y - kNavBarHeight;
    } completion:nil];
}
...
- (void)emojiBtnClick:(UIButton *)emojiBtn {
    ...
    if (self.moreBtn.selected) { // 工具+有选中
        ...
        if (!self.isShowingSystemKeyboard) {
            self.moreView.y = MainScreenHeight;
        }
        ...
        [UIView animateWithDuration:0.25 animations:^{
            self.y = MainScreenHeight - kChatEmojiHeight - self.height;
            _tableView.height = self.y - kNavBarHeight;
            
            emojiView.y = MainScreenHeight - kChatEmojiHeight;
        }];
...
- (void)moreBtnClick:(UIButton *)moreBtn {
    ...
    if (self.emojiBtn.selected) { // 表情键盘有选中
        ...
        // 添加动画
        [UIView animateWithDuration:0.25 animations:^{
            self.y = MainScreenHeight - kChatMoreHeight - self.height;
            _tableView.height = self.y - kNavBarHeight;
            moreView.y = MainScreenHeight - kChatMoreHeight;
        }];
...
- (void)resetState {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.y = MainScreenHeight - self.height - keyboardHeight;
        _tableView.height = self.y - kNavBarHeight;
        [self layoutIfNeeded];
    } completion:^(BOOL finished) {
        if (!finished) return;
...
- (KeyboardEmojiTextView *)textView {
    if (!_textView) {
        ...
        _textView.layer.borderColor = [UIColor colorWithHex:0xe1e1e5].CGColor;
...
- (LHChatBarMoreView *)moreView {
    if (!_moreView) {
        _moreView = [[LHChatBarMoreView alloc]initWithFrame:CGRectMake(0, MainScreenHeight, MainScreenWidth, kChatMoreHeight)];
        _moreView.backgroundColor = [UIColor colorWithHex:0xf8f8f8];
...
- (UIView *)emojiView {
    if (!_emojiView) {
        ...
        _emojiView.frame = CGRectMake(0, MainScreenHeight, MainScreenWidth, kChatEmojiHeight);
